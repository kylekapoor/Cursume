...
export default function ToolsPage() {
  ...
  const [docSource, setDocSource] = useState(""); // editable content
  const [fileType, setFileType] = useState<"latex" | "word" | null>("latex");
  ...
  return (
    <div className="h-screen w-full bg-[#f8f8f8] text-black overflow-hidden flex flex-col font-sans">
      ...
      <div className="w-full h-14 flex items-center justify-between px-6 bg-white border-b border-gray-200 shadow-sm relative">
        <Link href="/" className="text-lg font-semibold tracking-wide z-10">
          Cursume
        </Link>
        ...
        <div className="flex items-center space-x-3 z-10">
          <button
            ...
            className="text-sm px-3 py-1.5 bg-white text-black border border-gray-300 rounded-md hover:bg-gray-100"
          >
            Upload
          </button>
          <button
            onClick={handleDownload}
            className="bg-black text-white px-4 py-1.5 rounded-md hover:bg-gray-900 text-sm"
          >
            Download
          </button>
          <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-md shadow-lg p-6 w-full max-w-md relative">
            ...
            <p className="text-sm text-gray-600 mb-3">
              Supported File Types: PDF (.pdf), Word (.docx), LaTeX (.tex)
            </p>
            <input
              ref={fileInputRef}
              type="file"
        ...
        <div className="w-1/2 h-full border-r border-gray-200 bg-white">
          {advancedMode ? (
            <AdvancedEditor
              fileType={fileType}
              docSource={docSource}
              onChange={(newSource) => setDocSource(newSource)}
            />
          ) : (
            <AIPromptPanel docSource={docSource} setDocSource={setDocSource} />
          )}
        </div>

        <div className="w-1/2 h-full">
          <ResumePreview resumeHTML={docSource} />
        </div>
      </div>
    </div>
  );
